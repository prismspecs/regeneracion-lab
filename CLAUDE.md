...
* **Local nginx tweak (machine-local, not in the repo):** Local's generated nginx conf serves all CSS/JS with `Cache-Control: no-cache`, which forces a revalidation round-trip for every stylesheet on every navigation and gives Firefox a flash of unstyled content. Fix applied in `~/Local Sites/regeneracin-lab-newermaybe/conf/nginx/site.conf.hbs` (and the generated `site.conf` under `~/.config/Local/run/<id>/conf/nginx/`): theme CSS/JS (`/wp-content/themes/`) get `expires 1y / Cache-Control: public` — safe because the theme version-stamps every asset with `filemtime` (`?ver=`). Re-apply if the Local site is recreated.
