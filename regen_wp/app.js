class RegeneracionApp {
    constructor() {
        ...
        // Use the localized theme URL if available, otherwise fallback (though this is a WP theme so it should be there)
        this.themeUrl = window.regenWP ? window.regenWP.themeUrl : '';
        this.init();
    }
    ...
    async loadPageContent(page) {
        ...
        // Load content from HTML file in the theme pages directory
        try {
            const response = await fetch(`${this.themeUrl}/pages/${page}.html`);
            if (!response.ok) {
    ...
    loadInitialContent() {
        ...
        // Or we could load 'home' by default if we wanted
        else {
             this.navigateTo('home', false);
        }
    }
    ...
    renderContent(content) {
        ...
        this.resolveImagePaths(mainContent);
    }

    // Fix relative image paths in loaded content
    resolveImagePaths(container) {
        const images = container.querySelectorAll('img');
        images.forEach(img => {
            const src = img.getAttribute('src');
            if (src && !src.startsWith('http') && !src.startsWith('//') && !src.startsWith('data:')) {
                // If it's just 'images/filename.jpg', prepend the theme URL
                if (src.startsWith('images/')) {
                    img.src = `${this.themeUrl}/${src}`;
                } 
                // Handle cases where it might be relative like './images/'
                else if (src.startsWith('./images/')) {
                    img.src = `${this.themeUrl}/${src.substring(2)}`;
                }
            }
        });
        
        // Also handle inline styles with background images if any (e.g. hero)
        const elementsWithStyle = container.querySelectorAll('[style*="background-image"]');
        elementsWithStyle.forEach(el => {
            let style = el.getAttribute('style');
            if (style.includes('url(')) {
                 // This is a bit complex to parse reliably with regex, but for simple cases:
                 // url('images/foo.jpg') -> url('THEME_URL/images/foo.jpg')
                 // Only replace if it doesn't already have http/theme path
                 if (style.match(/url\(['"]?images\//)) {
                     const newStyle = style.replace(/url\(['"]?images\//g, `url('${this.themeUrl}/images/`);
    ...
    async loadPartials() {
        ...
        for (const placeholder of partials) {
            ...
            try {
                const response = await fetch(`${this.themeUrl}/partials/${partialName}.html`);
                if (response.ok) {
                    const content = await response.text();
                    placeholder.innerHTML = content;
                    // Resolve images in partials too
                    this.resolveImagePaths(placeholder);
                }
            } catch (error) {
